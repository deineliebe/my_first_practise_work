//Тут код с исправлениями после рецензий. Я не знаю, насколько корректно исправлять программу, уже отправленную на проверку, так что решила сделать это отдельно
...
void forth_task(double number)
{
    ...
    else
    {
        while ((save < 1) and (order > 0))
        {
            ...
            save *= 2;
            bool check = 1;
            //Было исправлено после получения первой рецензии
            d_cnt = 10;
            while (check)
            {
                if (order[d_cnt] == 0)
